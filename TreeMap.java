...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    protected int getAux(Position<Entry<K,V>> p){ return tree.getAux(p); }
     ...
     */
    protected ArrayList<Position<Entry<K,V>>> positions(){
        ArrayList<Position<Entry<K,V>>> arr= new ArrayList<>();
    ...
    public V put(K key, V value) throws IllegalArgumentException {
        ...
        if (isExternal(p)) {                    // key is new
            ...
            ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)p).setAux(tree.numChildren(p)+1);
            while(tree.parent(p)!=null&&size()!=1){
                ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)p).setAux(tree.numChildren(p)+1);
                p=tree.parent(p);
            }
            ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)tree.root).setAux(tree.numChildren(p)+1); //for root
             rebalanceInsert(p);// hook for balanced tree subclasses
            return null; // neden null döndük ne gerek var?
    ...
    public V remove(K key) throws IllegalArgumentException {
        ...
        if (isExternal(p)) {                    // key not found
            ...
            while (tree.root()!= copyOfP) {
                System.out.println((((BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.parent(copyOfP)).getAux()));
                ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)tree.parent(copyOfP)).setAux(((BalanceableBinaryTree.BSTNode<Entry<K,V>>)tree.parent(copyOfP)).getAux()-1);
                System.out.println((((BalanceableBinaryTree.BSTNode<Entry<K, V>>) tree.parent(copyOfP)).getAux()));
                copyOfP=tree.parent(copyOfP);
                System.out.println("gezililyor "+copyOfP.getElement().getKey());
            }
            ((BalanceableBinaryTree.BSTNode<Entry<K,V>>)tree.root).setAux(((BalanceableBinaryTree.BSTNode<Entry<K,V>>)tree.root).getAux()-1); // for root
            Position<Entry<K,V>> leaf = (isExternal(left(p)) ? left(p) : right(p));
            Position<Entry<K,V>> sib = sibling(leaf);
            remove(leaf);
            Position<Entry<K,V>> currentPosition=p;
    ...
    protected K findKthLargest(K k) {
        int th=0;
        Position<Entry<K,V>> currentPosition=tree.root();

        while (currentPosition.getElement().getKey() != k) {
            if (compare(currentPosition.getElement().getKey(), k) < 0) {
                th=tree.numChildren(tree.left(currentPosition))+1;//currentposition ve sol çocukların toplamı
               if(!isExternal(tree.right(currentPosition))){
                currentPosition=tree.right(currentPosition);//sağdan devam ediyoruz.
                   System.out.println("element "+currentPosition.getElement().getKey());
                }
            }else if (!isExternal(tree.left(currentPosition))) {
                    currentPosition = tree.left(currentPosition);
                System.out.println("element "+currentPosition.getElement().getKey());
            }
        }
        th+=tree.numChildren(tree.left(currentPosition));
        System.out.println(tree.size()-th-1+". büyük elemanın keyi "+currentPosition.getElement().getKey());
        return currentPosition.getElement().getKey();
    }
     public int depth(Position<Entry<K,V>> p) {
        return tree.depth(p);
     }
}
